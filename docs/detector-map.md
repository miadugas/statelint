...
| ✅     | Over-broad selector       | Zustand (bare `useStore()` / identity selector); Pinia (component-scope `$subscribe` — whole-store callback) | Narrow selector (zustand); `watch` a field / persist plugin (pinia)  |
